  ...
  try {
    ...
    for (const toolPage of TOOL_PAGES) {
      try {
        ...
        const getMetaContent = (name: string, attr = 'name'): string => {
        ...
        const ogDescription = getMetaContent('og:description', 'property');
        const ogImage = getMetaContent('og:image', 'property');
        const ogUrl = getMetaContent('og:url', 'property');
        ...
        const schemas = schemaMatches.map((match: string) => {
          try {
        ...
        const hasSoftwareSchema = !toolPage.isHub && schemas.some((s: any) => s['@type'] === 'SoftwareApplication');
        const hasBreadcrumbSchema = schemas.some((s: any) => s['@type'] === 'BreadcrumbList');
        ...
        const hasAdvancedSEO = hasCanonical && hasOgTags && hasRobotsMeta;
        if (!hasAdvancedSEO) {
        ...
        results.push({
          ...
          errors: [`Failed to fetch: ${(error as any).message}`],
          warnings: [],
          score: 0
        });
      }
