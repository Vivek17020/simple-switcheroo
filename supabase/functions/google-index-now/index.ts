...
// Ping unified sitemap index to Google and Bing
async function pingSitemapIndex(): Promise<boolean> {
  try {
    ...
    console.log('✅ Google pinged with sitemap-index.xml');
    
    // Ping Bing
    await fetch(`https://www.bing.com/ping?sitemap=${encodeURIComponent(sitemapIndexUrl)}`);
    console.log('✅ Bing pinged with sitemap-index.xml');
  ...
  try {
    ...
    if (pageType === 'article' && articleId) {
      ...
      if (article) {
        ...
        if (article.categories && Array.isArray(article.categories) && article.categories[0]) {
          urlsToIndex.push(`${BASE_URL}/category/${article.categories[0].slug}`);
        }
      }
    ...
    const backgroundTask = async () => {
      ...
      // 5. Ping unified sitemap index
      results.sitemapIndexPing = await pingSitemapIndex();
