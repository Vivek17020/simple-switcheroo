...
function generateAMPStory(story: WebStory, config: any): string {
  ...
  // Canonical URL should point to the non-AMP version with category
  const storyUrl = `${baseUrl}/webstories/${story.category.toLowerCase()}/${story.slug}`;
  const canonicalUrl = storyUrl; // Always use the non-AMP URL as canonical
...
<head>
  ...
  <!-- Canonical URL pointing to non-AMP version -->
