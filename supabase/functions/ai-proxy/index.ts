...
JSON OUTPUT (no code fences):`
          ...
          // Minimal post-processing: Only remove markdown fences (like Web3 articles)
          // NO aggressive regex that could break content or remove legitimate words
          if (parsed.formatted_content) {
            let fixedContent = parsed.formatted_content
            
            // Only safe cleanup - remove markdown code fences if present
            fixedContent = fixedContent.replace(/^```html\n?/gi, '')
            fixedContent = fixedContent.replace(/```\n?$/gi, '')
            fixedContent = fixedContent.trim()
            
            parsed.formatted_content = fixedContent
          }
...
Base suggestions on the article topic. If no authoritative sources are relevant, return empty array.`;
      ...
      case 'format-as-upsc':
        try {
          const upscPrompt = `You are an expert UPSC content editor. Transform this raw content into a structured UPSC exam-oriented article.

UPSC ARTICLE STRUCTURE (MANDATORY):

1. **Short Summary** (2-3 lines)
   - Key takeaway in simple language
   - Format: <div class="upsc-summary"><p>Summary text here...</p></div>

2. **Context / Background**
   - <h2 id="context">Context / Background</h2>
...
3. **Main Explanation**
   ...
   - Use sub-headings (H3) for sub-topics
   - Tables where comparison needed

4. **Important Features / Key Points**
   - <h2 id="key-points">Key Points</h2>
   - Use <ul> bullet list
   - 5-10 important points
   - Exam-focused highlights

5. **UPSC Notes Version** (CRITICAL - Ultra-concise bullet points)
   - <h2 id="upsc-notes">UPSC Notes</h2>
   - <div class="upsc-notes-box">
       <ul>
         <li>Point 1</li>
         <li>Point 2</li>
         ...
...
6. **Mains Answer Writing Angle**
   - <h2 id="mains-perspective">Mains Answer Writing Angle</h2>
   - GS Paper relevance (I/II/III/IV)
   - Possible question types
   - Key points to include in answers
   - Word limits suggestions (150/250 words)

7. **Prelims MCQs** (2-3 questions)
   - <h2 id="practice-questions">Prelims Practice Questions</h2>
   - Format each MCQ as:
   <div class="upsc-mcq">
     <p class="question"><strong>Q1:</strong> Question text?</p>
     <ol type="a">
       <li>Option A</li>
       <li class="correct"><strong>Option B ✓</strong></li>
       <li>Option C</li>
       <li>Option D</li>
     </ol>
     <details><summary>Show Answer</summary><p><strong>Answer: B.</strong> Explanation here...</p></details>
   </div>

8. **Previous Year Question Mapping** (if applicable)
   - <h2 id="pyq-mapping">Previous Year Questions</h2>
   - Related questions from past UPSC exams
   - Format: <div class="upsc-pyq">
       <p><strong>UPSC 2023:</strong> Related question text...</p>
     </div>

9. **Conclusion**
   - <h2 id="conclusion">Conclusion</h2>
   - 2-3 lines simple summary
   - Way forward if applicable

TONE & STYLE:
- Clear, factual, exam-oriented
- No fluff or unnecessary words
- Use simple language
- Active voice preferred
- Include relevant keywords for SEO

RETURN ONLY HTML CONTENT. No code fences, no markdown, no explanations.

CONTENT TO FORMAT:
${content}

FORMATTED UPSC-STYLE HTML:`;

          const upscContent = await callLovableAI(upscPrompt);
          
          // Clean any code fences
          let cleaned = upscContent.trim();
          cleaned = cleaned.replace(/^```html\n?/i, '');
          cleaned = cleaned.replace(/^```\n?/i, '');
          cleaned = cleaned.replace(/\n?```$/i, '');
          ...
          console.error('UPSC format error:', error);
          throw new Error('Failed to format content for UPSC');
        }
        break
