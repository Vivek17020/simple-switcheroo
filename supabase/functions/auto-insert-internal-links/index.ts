...
serve(async (req) => {
  ...
  try {
    const { content, title, currentArticleId, categoryId, parentCategoryId }: InternalLinkRequest = await req.json()
    ...
    console.log('Category filter:', { categoryId, parentCategoryId })

    // Get all category IDs to search (current + siblings if subcategory)
    let categoryIds: string[] = []
    
    if (categoryId) {
      categoryIds.push(categoryId)
      
      // If this is a subcategory, also get sibling categories (same parent)
      if (parentCategoryId) {
        ...
        if (siblingCategories) {
          categoryIds = [...categoryIds, ...siblingCategories.map(c => c.id)]
        }
        // Also include parent category itself
    ...
    // Query database for related articles within same category family
    let query = supabase
      .from('articles')
