    ...
    const aiResponse = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a visual storytelling expert for web stories (AMP stories). Transform content into 6-7 compelling slides.
...
Return ONLY valid JSON in this format:
{
  ...
  "keywords": ["keyword1", "keyword2"]
    ...
    // Generate images for each slide using Lovable AI with caching
    const slidesWithImages = await Promise.all(
      storyData.slides.map(async (slide: SlideData, index: number) => {
        try {
          // Enhanced prompt without text overlay
          const enhancedPrompt = `Create a vertical 9:16 aspect ratio image for a web story slide. ${slide.imagePrompt}. NO TEXT, NO OVERLAY, NO CAPTIONS on the image. Professional, high-quality, clean visual with vivid colors and sharp details.`;
    ...
    const { data: story, error: createError } = await supabase
      ...
      .insert({
        ...
        user_id: userId || null, // Allow NULL for auto-generated stories
        auto_generated: true,
