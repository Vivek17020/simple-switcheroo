...
export const usePersonalizedFeed = (limit = 12) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // Get web3forindia category to exclude it
      ...
      // Exclude web3 articles
      if (web3CategoryIds.length > 0) {
      ...
      if (preferences?.preferred_categories?.length) {
        // Filter out web3 categories from user preferences
        const filteredPreferredCategories = preferences.preferred_categories.filter(
          (catId: string) => !web3CategoryIds.includes(catId)
        );

        // Get preferred articles first
        const { data: preferredArticles, error: prefError } = await query
          .in("category_id", filteredPreferredCategories)
        ...
        // Exclude web3 articles from trending
        if (web3CategoryIds.length > 0) {
          trendingQuery = trendingQuery.not('category_id', 'in', `(${web3CategoryIds.join(',')})`);
        }

        const { data: trendingArticles, error: trendError } = await trendingQuery;
...
export const useEnhancedRelatedArticles = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // Get web3forindia category to check if current article is web3
      const { data: web3Category } = await supabase
        .from('categories')
        .select('id')
        .eq('slug', 'web3forindia')
      ...
      // Get web3 subcategories
      let web3CategoryIds: string[] = [];
      let isWeb3Article = false;
      if (web3Category) {
        const { data: web3Subcategories } = await supabase
          .from('categories')
          .select('id')
          .eq('parent_id', web3Category.id);
        web3CategoryIds = [web3Category.id, ...(web3Subcategories || []).map(c => c.id)];
        isWeb3Article = web3CategoryIds.includes(categoryId);
      }
      ...
      const { data: categoryArticles, error: catError } = await categoryQuery;
      ...
      if (tags.length > 0) {
        ...
        // If this is NOT a web3 article, exclude web3 articles from related articles
        if (!isWeb3Article && web3CategoryIds.length > 0) {
          tagQuery = tagQuery.not('category_id', 'in', `(${web3CategoryIds.join(',')})`);
        }
