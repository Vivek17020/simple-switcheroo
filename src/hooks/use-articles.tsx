...
export const useArticles = (categorySlug?: string, page = 1, limit = 12, sortOrder?: 'oldest' | 'newest') => {
  return useQuery({
    ...
    queryFn: async () => {
      // Get web3forindia category to exclude it from main homepage
      ...
      } else if (web3Category) {
        // Exclude Web3 articles from main homepage
        const { data: web3Subcategories } = await supabase
          .from('categories')
          .select('id')
          .eq('parent_id', web3Category.id);

        const web3CategoryIds = [web3Category.id, ...(web3Subcategories || []).map(c => c.id)];
