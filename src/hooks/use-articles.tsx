...
export const useArticles = (categorySlug?: string, page = 1, limit = 12, sortOrder?: 'oldest' | 'newest') => {
  return useQuery({
    ...
    queryFn: async () => {
      // Get web3forindia and upscbriefs categories to exclude from main homepage
      const [{ data: web3Category }, { data: upscCategory }] = await Promise.all([
        supabase.from('categories').select('id').eq('slug', 'web3forindia').maybeSingle(),
        supabase.from('categories').select('id').eq('slug', 'upscbriefs').maybeSingle()
      ]);
      let query = supabase
        .from("articles")
        .select(`
      ...
      if (categorySlug) {
        ...
        // Exclude Web3 and UPSC articles from main homepage
        const excludeCategoryIds: string[] = [];
        ...
        if (web3Category) {
          ...
          excludeCategoryIds.push(web3Category.id, ...(web3Subcategories || []).map(c => c.id));
        }
        
        if (upscCategory) {
          const { data: upscSubcategories } = await supabase
            .from('categories')
            .select('id')
            .eq('parent_id', upscCategory.id);
          excludeCategoryIds.push(upscCategory.id, ...(upscSubcategories || []).map(c => c.id));
        }
      ...
      if (authorIds.length > 0) {
        try {
          ...
          console.warn("Could not fetch author profiles:", error);
        }
      }
