...
export default function Web3CategoryPage() {
  ...
  return (
    <>
      <Helmet>
        <title>{category?.name || "Category"} Tutorials | Web3 for India - Learn Blockchain & Smart Contracts</title>
        <meta 
          name="description" 
          content={`${category?.description || `Learn ${category?.name}`} - Comprehensive Web3 tutorials, blockchain guides, smart contracts, DeFi, NFTs, and cryptocurrency education for Indians.`}
        />
        <meta 
          name="keywords" 
          content={`${category?.name}, web3 tutorial, blockchain guide, smart contracts india, ${categorySlug}, cryptocurrency learning`}
        />
        <link rel="canonical" href={`https://www.thebulletinbriefs.in/web3forindia/${categorySlug}`} />
        <meta property="og:title" content={`${category?.name} - Web3 for India`} />
        <meta property="og:description" content={category?.description || "Web3 tutorials and guides"} />
      ...
      <Web3StructuredData
        ...
        data={{
          ...
          modules: articles?.slice(0, 5).map((article: any) => ({
            ...
            url: `https://www.thebulletinbriefs.in/web3forindia/article/${article.slug}`,
          })),
        }}
      />
