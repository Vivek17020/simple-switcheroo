...
import { Loader2, Newspaper, Smartphone, Sparkles, GraduationCap } from 'lucide-react';
...
export function RichTextEditor({ content, onChange, placeholder }: RichTextEditorProps) {
  ...
  const uploadImage = useCallback(async (file: File) => {
    try {
      toast({
        ...
        description: "Optimizing and uploading to Cloudinary CDN",
      });

      const { uploadContentImage, formatFileSize } = await import('@/lib/cloudinary-upload');
      
      const result = await uploadContentImage(file, (progress) => {
        console.log(`Upload progress: ${progress}%`);
      });

      toast({
        title: "Image uploaded!",
        description: `Size: ${formatFileSize(result.bytes || 0)} | CDN optimized`,
      });
  ...
  const handleFormatAsUPSC = useCallback(async () => {
    if (!editor) return;
    ...
    if (!currentContent || currentContent === '<p></p>') {
      toast({
        title: "No content",
        description: "Please add content first before formatting for UPSC",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (data?.result) {
        editor.commands.setContent(data.result);
        toast({
          title: "Formatted for UPSC",
          description: "Content has been reformatted in UPSC exam-oriented style",
        });
      }
    } catch (error: any) {
      console.error('Error formatting for UPSC:', error);
      toast({
        title: "Error",
        description: error.message || "Failed to format content for UPSC",
        variant: "destructive",
      });
    } finally {
      setIsFormattingNews(false);
        ...
        {!markdownMode && editor && (
          <>
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={handleFormatAsUPSC}
              disabled={isFormattingNews}
              className="text-blue-800 dark:text-blue-300 bg-gradient-to-r from-blue-900/10 to-blue-600/10 hover:from-blue-900/20 hover:to-blue-600/20"
              title="Format for UPSC (Exam-Oriented Style)"
            >
              {isFormattingNews ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
