...
import { Upload, X, Plus, Eye, Smartphone, CheckCircle2 } from 'lucide-react';
import slugify from 'slugify';
import { WebStorySlide } from '@/hooks/use-web-stories';
import { WebStoryAMPValidator } from './web-story-amp-validator';
...
export function WebStoryForm({ initialData, onSave }: WebStoryFormProps) {
  ...
  return (
    <div className="space-y-6">
          ...
          {slides.map((slide, index) => (
            <Card key={index}>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <Label htmlFor={`text-${index}`}>Text Overlay</Label>
                  <Textarea
                    id={`text-${index}`}
                    value={slide.text}
                  ...
                  <div key={index} className="relative flex-shrink-0 w-48 aspect-[9/16] bg-card rounded-lg overflow-hidden border-2 border-border shadow-lg">
                    ...
                    {slide.text && (
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                        <p className="text-white text-sm font-medium">{slide.text}</p>
                      </div>
                    )}
      ...
      {/* AMP Validation Section - Only show for saved stories */}
      {initialData?.id && title && category && (
        <Card className="border-primary/20">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <CheckCircle2 className="w-5 h-5 text-primary" />
              AMP Story Validation
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <Alert>
              <AlertDescription>
                Validate your Web Story to ensure it meets Google's AMP requirements for Search and Discover.
                This validation checks the generated AMP HTML for compliance.
              </AlertDescription>
            </Alert>
            
            <WebStoryAMPValidator
              storySlug={slugify(title, { lower: true, strict: true })}
              storyTitle={title}
            />
            
            <div className="text-sm text-muted-foreground">
              <p className="font-medium mb-2">What's checked:</p>
              <ul className="list-disc list-inside space-y-1 ml-2">
                <li>Required AMP HTML structure and tags</li>
                <li>Mandatory AMP scripts inclusion</li>
                <li>Canonical URL configuration</li>
                <li>Publisher metadata completeness</li>
                <li>Image and content formatting</li>
              </ul>
            </div>
          </CardContent>
        </Card>
