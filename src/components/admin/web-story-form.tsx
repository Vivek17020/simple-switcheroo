...
export function WebStoryForm({ initialData, onSave }: WebStoryFormProps) {
  ...
  const handleImageUpload = async (index: number, file: File) => {
    ...
    try {
      const { uploadWebStoryImage } = await import('@/lib/cloudinary-upload');

      const result = await uploadWebStoryImage(file, (progress) => {
        console.log(`Slide ${index + 1} upload: ${progress}%`);
      });
      ...
      toast({
        ...
        description: 'Slide image uploaded to Cloudinary CDN',
      });
    } catch (error) {
