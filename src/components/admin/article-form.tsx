...
export function ArticleForm({ article, onSave }: ArticleFormProps) {
  ...
  const handleImageUpload = async (file: File) => {
    try {
      const { uploadFeaturedImage, formatFileSize } = await import('@/lib/cloudinary-upload');
      const { getImageDimensions, meetsGoogleDiscoverRequirements } = await import('@/lib/image-optimizer');
      ...
      if (!isGoogleDiscoverReady) {
        toast({
          ...
          description: `Current: ${dimensions.width}x${dimensions.height}px. Google Discover requires minimum 1200px width.`,
          variant: "destructive",
        });
      }
      ...
      toast({
        ...
        description: "Optimizing and uploading your image",
      });

      const result = await uploadFeaturedImage(file, (progress) => {
      ...
      toast({
        ...
        description: `${discoverStatus} | Size: ${formatFileSize(result.bytes || 0)}`,
      });
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      // Apply auto-repairs but don't block publishing
      // Content validation is available via "Validate & Fix" button for manual use
      if (!isDraft) {
        const validation = validateAndRepairHtml(formData.content);
        // Apply any auto-repairs but allow publishing regardless
        if (validation.stats.changesApplied > 0) {
