...
import { InternalLinkSuggester } from './internal-link-suggester';
import { supabase } from '@/integrations/supabase/client';
import { toast } from '@/hooks/use-toast';
import { Upload, Eye, Save, Send, X, Clock, CheckCircle, Youtube, Sparkles, ClipboardCheck, Target } from 'lucide-react';
...
export function ArticleForm({ article, onSave }: ArticleFormProps) {
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      // Content validation - Check sanitization impact
      const originalContentLength = formData.content.length;
      const sanitizedContent = sanitizeHtml(formData.content);
      const sanitizedContentLength = sanitizedContent.length;
      const contentLoss = ((originalContentLength - sanitizedContentLength) / originalContentLength) * 100;

      if (contentLoss > 10) {
        console.warn('⚠️ Significant content loss during sanitization:', {
          original: originalContentLength,
          sanitized: sanitizedContentLength,
          loss: contentLoss.toFixed(2) + '%'
        });
        toast({
          title: 'Content Warning',
          description: `${contentLoss.toFixed(0)}% of content was removed during sanitization. Check console for details.`,
          variant: 'default',
        });
      } else if (contentLoss > 0) {
      ...
      if (!isDraft) {
        ...
        if (!formData.meta_title) {
          ...
          errors.push(`Meta title is too long (${formData.meta_title.length}/60 characters)`);
        }
        
        if (!formData.meta_description) {
          errors.push('Meta description is required');
        } else if (formData.meta_description.length > 160) {
          errors.push(`Meta description is too long (${formData.meta_description.length}/160 characters)`);
        }
        
        if (contentLength < 300) {
          errors.push(`Content is too short (${contentLength}/300 characters minimum)`);
        }
        
        if (errors.length > 0) {
          toast({ 
            title: 'Cannot Publish - SEO Requirements Not Met', 
      ...
      const articleData = {
        ...
        secondary_keywords: formData.secondary_keywords || null,
        lsi_keywords: formData.lsi_keywords || null,
        target_queries: formData.target_queries || null,
        keyword_density: formData.keyword_density || null,
      } as any;
    ...
    try {
      ...
      if (data) {
        ...
        if (data.formatted_content) {
          ...
          // CRITICAL: Sanitize content immediately to match what will be displayed
          const sanitized = sanitizeHtml(limited);
  ...
  const handleHumanizeContent = async () => {
    ...
    try {
      ...
      if (data?.result) {
        ...
        // Sanitize content immediately to match what will be displayed
        const sanitized = sanitizeHtml(cleaned);
        updateFormData({ content: sanitized });
        toast({
  ...
  const handleAnalyzeKeywords = async () => {
    if (!formData.title?.trim() || !formData.content?.trim()) {
      toast({
        ...
        description: "Please add title and content before analyzing keywords.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke('analyze-keywords', {
        body: {
          title: formData.title,
          content: formData.content,
          articleId: article?.id, // Auto-save to DB if editing
        }
      });
      ...
      if (data?.analysis) {
        ...
        // Update form with keyword data
        updateFormData({
          primary_keyword: data.analysis.primaryKeyword,
          secondary_keywords: data.analysis.secondaryKeywords,
          lsi_keywords: data.analysis.lsiKeywords,
          target_queries: data.analysis.targetQueries,
          keyword_density: data.analysis.keywordDensity,
        });
        
        toast({
          title: "✨ Keywords Analyzed",
          description: `Found ${data.analysis.secondaryKeywords?.length || 0} secondary keywords and ${data.analysis.lsiKeywords?.length || 0} LSI keywords.`,
        });
      }
    } catch (error: any) {
  ...
  const handleApplyOptimization = async () => {
    if (!formData.content?.trim() || !keywordAnalysis) {
      toast({
        title: "No Analysis",
        description: "Please analyze keywords first before applying optimization.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      // Build a detailed prompt with the recommendations
      const recommendations = keywordAnalysis.recommendations || [];
      ...
      const { data, error } = await supabase.functions.invoke('ai-proxy', {
        body: {
          task: 'seo-optimize',
          content: formData.content,
          title: formData.title + recommendationsText,
        }
      });
      ...
      if (data?.result) {
        ...
        toast({
          ...
          description: "Article has been optimized based on keyword analysis recommendations.",
        });
      }
    } catch (error: any) {
      ...
      toast({
        title: "Optimization Failed",
        description: error?.message || "Failed to apply optimization. Please try again.",
        variant: "destructive",
      });
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <CardHeader>
              <div className="space-y-4">
                ...
                <Button
                  ...
                  disabled={isAnalyzingKeywords || !formData.title?.trim() || !formData.content?.trim()}
                  className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 hover:from-blue-500/20 hover:to-cyan-500/20"
                >
                  <Target className="w-4 h-4 mr-2" />
                  {isAnalyzingKeywords ? 'Analyzing...' : '🔍 Analyze Keywords'}
                </Button>
                ...
                {keywordAnalysis && (
                  ...
                  >
                    <Sparkles className="w-4 h-4 mr-2" />
                    {isApplyingOptimization ? 'Optimizing...' : '✨ Apply Optimization'}
                  </Button>
                )}
              </div>
            ...
            <CardContent>
              ...
              <div className="flex items-center justify-between mt-2 pt-2 border-t border-border/50">
                ...
                <span className={`text-xs font-medium ${
                  ...
                  {formData.content.replace(/<[^>]*>/g, '').length < 300 && ' (Min 300 required for publishing)'}
                </span>
              </div>
            </CardContent>
          </Card>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs ${
                    ...
                    {!formData.meta_title && ' (Required for publishing)'}
                    {formData.meta_title && formData.meta_title.length > 60 && ' (Too long)'}
                  </span>
                </div>
                <Input
                  id="meta_title"
                  ...
                  placeholder="SEO title (required for publishing)"
                  className={`mt-1 ${
                    formData.meta_title && formData.meta_title.length > 60 
                      ? 'border-destructive' 
                      : ''
                  }`}
                />
                  ...
                  <span className={`text-xs ${
                    !formData.meta_description 
                      ...
                      : formData.meta_description.length > 160 
                        ? 'text-destructive' 
                        : formData.meta_description.length > 140 
                          ? 'text-yellow-600' 
                          : 'text-green-600'
                  }`}>
                    {formData.meta_description?.length || 0}/160
                    {!formData.meta_description && ' (Required for publishing)'}
                    {formData.meta_description && formData.meta_description.length > 160 && ' (Too long)'}
                  </span>
                </div>
                <Textarea
                  id="meta_description"
                  ...
                  placeholder="SEO description (required for publishing)"
                  className={`mt-1 ${
        ...
        <div className="space-y-6">
          ...
          <SEOContentAnalyzer
            title={formData.title}
            content={formData.content}
            primaryKeyword={formData.primary_keyword}
            secondaryKeywords={formData.secondary_keywords}
            lsiKeywords={formData.lsi_keywords}
            targetQueries={formData.target_queries}
            keywordDensity={formData.keyword_density}
          />
