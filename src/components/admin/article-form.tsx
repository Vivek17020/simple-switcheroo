...
import { DiscoverOptimizationModal } from './discover-optimization-modal';
...
export function ArticleForm({ article, onSave }: ArticleFormProps) {
  ...
  const [isGatheringNews, setIsGatheringNews] = useState(false);
  ...
  const [isFormattingDiscover, setIsFormattingDiscover] = useState(false);
  ...
  // Category type state
  const [articleType, setArticleType] = useState<'regular' | 'web3' | 'upsc'>('regular');
  ...
  const handleGenerateFeaturedImage = async () => {
    if (!formData.title.trim()) {
      toast({
        ...
        description: "Please enter an article title first to generate a relevant image.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      toast({
        ...
        description: "Creating an eye-catching featured image with AI",
      });
      ...
      if (data?.imageUrl) {
        ...
        // Set the image preview and form data
        setImagePreview(uploadedUrl);
        updateFormData({ image_url: uploadedUrl });
        
        toast({
          title: "✅ Image generated!",
          description: "AI-generated featured image has been added to your article.",
        });
      } else {
  ...
  const handleImageUpload = async (file: File) => {
    try {
      ...
      const { optimizeFeaturedImage, formatFileSize, getImageDimensions, meetsGoogleDiscoverRequirements } = await import('@/lib/image-optimizer');
      
      // Check image dimensions first
      const dimensions = await getImageDimensions(file);
      const isGoogleDiscoverReady = meetsGoogleDiscoverRequirements(dimensions);
      
      if (!isGoogleDiscoverReady) {
        toast({
          title: "⚠️ Image too small for Google Discover",
          description: `Current: ${dimensions.width}x${dimensions.height}px. Google Discover requires minimum 1200px width. Consider using AI Generate for optimal images.`,
          variant: "destructive",
        });
      }
      ...
      toast({
        title: "Optimizing featured image...",
        description: "Preparing image for Google Discover compatibility",
      });

      // Use featured image optimizer (preserves 1200px+ width)
      const optimized = await optimizeFeaturedImage(file, (progress) => {
        console.log(`Featured image optimization: ${progress}%`);
      });
      ...
      // Show optimization results with Google Discover status
      const discoverStatus = isGoogleDiscoverReady ? '✅ Google Discover ready' : '⚠️ Below recommended size';
      toast({
        title: "Featured image uploaded!",
        description: `${discoverStatus} | Reduced by ${optimized.compressionRatio}% (${formatFileSize(optimized.originalSize)} → ${formatFileSize(optimized.optimizedSize)})`,
      });
    ...
    try {
      ...
      if (data) {
        ...
        if (data.formatted_content) {
          // Minimal cleanup - only remove markdown fences (like Web3 articles)
          // NO aggressive regex that could break content, remove keywords, or alter structure
  ...
  const handleFormatDiscover = async () => {
    if (!formData.content?.trim()) {
      toast({
        title: "No Content",
        description: "Please add content first to optimize for Google Discover.",
        variant: "destructive",
      });
      return;
    }

    setIsFormattingDiscover(true);
    
    try {
      // Check image before optimizing
      if (!formData.image_url) {
        toast({
          title: "⚠️ No Featured Image",
          description: "Google Discover requires high-quality images (≥1200px). Consider adding one first.",
          duration: 5000,
        });
      }

      const { data, error } = await supabase.functions.invoke('format-discover-article', {
        body: {
      ...
      if (data && !data.error) {
        // Store the data and open the modal for user review
        setDiscoverData(data);
        setDiscoverModalOpen(true);
        
        toast({
          title: "✨ Discover Analysis Complete!",
          description: `Probability: ${data.discover_probability || 0}% - Review and apply optimizations.`,
          duration: 5000,
        });
      } else if (data?.error) {
        toast({
          title: "Formatting Issue",
          description: data.error + '. Try again with shorter content.',
          variant: "destructive",
        });
        return;
      }
    } catch (error: any) {
      console.error('Discover format error:', error);
      toast({
        title: "Formatting Failed",
        description: error?.message || "Failed to optimize for Google Discover.",
        variant: "destructive",
      });
    } finally {
      setIsFormattingDiscover(false);
    }
  };

  const handleApplyDiscoverOptimizations = (title: string, metaDescription: string, content: string) => {
    updateFormData({
      title,
      meta_title: title,
      meta_description: metaDescription,
      content,
      excerpt: discoverData?.key_highlights?.slice(0, 3).join(' • ') || formData.excerpt,
    });
  };
...
<pre><code class="language-${language}">${code.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</code></pre>
  ...
  const handleAIGatherNews = async () => {
    setIsGatheringNews(true);
    
    try {
      toast({
        title: "Gathering Breaking News...",
        description: "AI is fetching the latest trending news stories for you.",
      });

      const { data, error } = await supabase.functions.invoke('ai-gather-news', {
        body: {
          count: 1
        }
      });
      ...
      if (data?.news) {
        ...
        // Populate the editor with the gathered news
        updateFormData({
        ...
        toast({
          title: "News Gathered!",
          description: `${newsItem.title} - Now formatting...`,
        });

        // Wait a bit for the content to be set
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // Auto-trigger format as news
        handleFormatNews();
      } else {
        toast({
          title: "No News Found",
          description: "Could not find breaking news at this moment. Try again later.",
          variant: "destructive",
        });
      }
    } catch (error: any) {
      console.error('AI gather news error:', error);
      toast({
        title: "News Gathering Failed",
        description: error?.message || "Failed to gather breaking news.",
        variant: "destructive",
      });
    } finally {
      setIsGatheringNews(false);
    }
  };
  ...
  const handleInjectLinks = async () => {
    ...
    if (!formData.category_id) {
      toast({
        ...
        description: "Please select a category first to find related articles.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      // Get current category and its parent for broader matching
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <CardHeader>
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {/* Google Discover Formatter */}
                  <Button
                    size="default"
                    onClick={handleFormatDiscover}
                    disabled={isFormattingDiscover || !formData.content?.trim()}
                  ...
                  >
                    <Sparkles className="w-4 h-4 mr-2" />
                    {isFormattingDiscover ? 'Optimizing for Discover...' : '🚀 Discover Format'}
                  </Button>
                ...
                >
                  ...
                  <Button
                    size="default"
                    onClick={handleAIGatherNews}
                    disabled={isGatheringNews}
                    className="bg-gradient-to-r from-rose-600 to-orange-600 hover:from-rose-700 hover:to-orange-700 text-white transition-all duration-300 shadow-lg"
                  >
                    <Sparkles className="w-4 h-4 mr-2" />
                    {isGatheringNews ? 'Gathering News...' : '🤖 AI Gather & Format News'}
                  </Button>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex gap-2 flex-wrap">
                ...
                <Button
                  type="button"
                  variant={articleType === 'upsc' ? 'default' : 'outline'}
                  onClick={() => {
                    setArticleType('upsc');
                    updateFormData({ category_id: '' });
                  }}
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                {articleType === 'web3' ? 'Web3ForIndia Category' : 
                 articleType === 'upsc' ? 'UPSC Briefs Category' : 'Category'}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <Select
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder={`Select ${articleType === 'web3' ? 'Web3ForIndia' : articleType === 'upsc' ? 'UPSC Briefs' : 'a'} category`} />
                </SelectTrigger>
                <SelectContent>
                  {categories
                    .filter((category) => {
                      ...
                      } else if (articleType === 'upsc') {
                        // Show only UPSCBriefs subcategories
                        const parentCategory = categories.find(cat => cat.id === category.parent_id);
                        return parentCategory?.slug === 'upscbriefs';
                      } else {
                        // Regular articles: exclude Web3 and UPSC categories
                        if (!category.parent_id) {
                          return category.slug !== 'web3forindia' && category.slug !== 'upscbriefs';
                        }
                        const parentCategory = categories.find(cat => cat.id === category.parent_id);
                        return parentCategory?.slug !== 'web3forindia' && parentCategory?.slug !== 'upscbriefs';
                      }
                    })
                    .map((category) => (
      ...
      {/* Discover Optimization Modal */}
      <DiscoverOptimizationModal
        open={discoverModalOpen}
        onOpenChange={setDiscoverModalOpen}
        data={discoverData}
        onApply={handleApplyDiscoverOptimizations}
      />
    </div>
  );
}