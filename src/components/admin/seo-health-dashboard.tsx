...
export function SeoHealthDashboard() {
  ...
  // GSC Error Prevention mutation
  const preventGscMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.functions.invoke('prevent-gsc-errors');
      if (error) throw error;
      return data;
    },
    onSuccess: (data) => {
      toast.success(`Prevented ${data.preventedErrors} GSC errors, requested ${data.indexingRequested} indexing updates`);
      queryClient.invalidateQueries({ queryKey: ['seo-health-issues'] });
    },
    onError: (error: any) => {
      toast.error('GSC prevention failed: ' + error.message);
    }
  });
  ...
  const getIssueTypeLabel = (issueType: string) => {
    const labels: Record<string, string> = {
      ...
      'crawled_not_indexed': '🕷️ Crawled - Not Indexed',
      'page_with_redirect': '↪️ Page with Redirect',
      'stale_content': '⏰ Stale Content',
      'not_indexed_intentionally': 'ℹ️ Noindex Set'
    };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            onClick={() => preventGscMutation.mutate()}
            disabled={preventGscMutation.isPending}
            variant="default"
            className="bg-green-600 hover:bg-green-700"
          >
            {preventGscMutation.isPending ? (
              <>
