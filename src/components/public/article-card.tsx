...
export function ArticleCard({ article, featured = false, compact = false }: ArticleCardProps) {
  ...
  if (featured) {
    return (
      <Link to={`/article/${article.slug}`} className="group block" onMouseEnter={() => import("@/pages/ArticlePage")}>
            ...
            <div className="absolute top-4 left-4">
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  if (article.categories?.slug) navigate(`/category/${article.categories.slug}`);
                }}
                className="focus:outline-none"
                aria-label={`View ${article.categories?.name} category`}
              >
                <Badge 
